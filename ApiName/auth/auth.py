...
def register():
    ...
    try:
        ...
        refresh_token = create_refresh_token(
            identity=new_user.id, expires_delta=timedelta(days=90)
        )
...
def resend_otp():
    ...
    user.otp = new_otp
    otp_expiry = datetime.now() + timedelta(
        minutes=10
...
def refresh():
    ...
    return (
        jsonify(
            ...
            user_data=User.query.get(current_user_id).format(),
        ),
        200,
    )
...
def change_password(user):
    ...
    if data["old_password"] == data["new_password"]:
        return (
            jsonify({"message": "Old password and new password cannot be the same"}),
            400,
        )
